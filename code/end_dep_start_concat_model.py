...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        def shared_output_nn(blended_reps, attn_start_hidden_reps=None, reuse=None):
            """
            Output Neural Nets with shared weights.

            Inputs:
              blended_reps: shape (batch_size, context_len, 4*hidden_size).
              attn_start_hidden_reps: shape (batch_size, 1, hidden_size).
            ...
            with tf.variable_scope('FirstHidden') as hidden_scope:
                blended_hidden_reps = tf.contrib.layers.fully_connected(
                    blended_reps,
                    num_outputs=self.FLAGS.hidden_size,
                    reuse=reuse,
                    scope=hidden_scope) # blended_hidden_reps is shape (batch_size, context_len, hidden_size)

                if attn_start_hidden_reps is not None:
                    blended_hidden_reps = tf.add(blended_hidden_reps, attn_start_hidden_reps, name='blended_hidden_reps_w_attn_start')
            ...
            with tf.variable_scope('SimpleSoftmax') as softmax_scope:
                ...
                return softmax_layer.build_graph(
                    blended_hidden_reps, self.context_mask,
                    reuse=reuse,
                    scope=softmax_scope)
        ...
        with tf.variable_scope('StartEnd') as se_scope:
            ...
            attn_start_hidden_reps = tf.contrib.layers.fully_connected(attn_start, num_outputs=self.FLAGS.hidden_size)  # shape (batch_size, 1, hidden_size)
            self.logits_end, self.probdist_end = shared_output_nn(blended_reps, attn_start_hidden_reps, reuse=True)
