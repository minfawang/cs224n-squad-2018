...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('CoAttnGru'):
        ...
        with tf.variable_scope('SelfAttnGru'):
            self_attn_in = tf.concat([bidaf_output, coattn_output], 2)  # (batch_size, context_len, hidden_size*4)
            self_attn_layer = MulAttn(self.keep_prob, self.FLAGS.hidden_size * 4, self.FLAGS.hidden_size * 4)
            self_attn_output = self_attn_layer.build_graph(self_attn_in, self.context_mask, self_attn_in, self.context_mask)  # (batch_size, context_len, hidden_size*4)
            self_attn_output = tf.contrib.layers.fully_connected(
                self_attn_output,
                num_outputs=self.FLAGS.hidden_size,
                normalizer_fn=tf.contrib.layers.batch_norm
            )  # (batch_size, context_len, hidden_size)

            self_attn_gru = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            self_attn_output = self_attn_gru.build_graph(self_attn_output, self.context_mask)  # (batch_size, context_len, hidden_size*2)

        model_reps = tf.concat([bidaf_output, coattn_output, self_attn_output], 2)  # (batch_size, context_len, hidden_size*6)
