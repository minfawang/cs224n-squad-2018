...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, MulAttn, BidafAttn, SelfAttn, CoAttn
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('BidafGru'):
            ...
            bidaf_gru_layer = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            bidaf_output = bidaf_gru_layer.build_graph(bidaf_output, self.context_mask)  # (batch_size, context_len, hidden_size*2)

        with tf.variable_scope('CoContextGru'):
            coattn_layer = CoAttn(self.keep_prob, self.FLAGS.hidden_size, self.FLAGS.hidden_size * 2)
            coattn_output = coattn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # coattn_output is shape (batch_size, context_len, hidden_size*2)

        with tf.variable_scope('CoQuestionGru'):
            coqn_layer = CoAttn(self.keep_prob, self.FLAGS.hidden_size, self.FLAGS.hidden_size * 2)
            coqn_out = coqn_layer.build_graph(bidaf_output, self.context_mask, question_hiddens, self.qn_mask)  # (batch_size, question_len, hidden_size * 2)
            tf.assert_equal(coqn_out.get_shape().as_list()[1:], [self.FLAGS.question_len, self.FLAGS.hidden_size * 2])

        with tf.variable_scope('CoCoGru'):
            coco_in = tf.concat([bidaf_output, coattn_output], 2)  # (batch_size, context_len, hidden_size*4)
            coco_in = tf.contrib.layers.fully_connected(
                coco_in, num_outputs=self.FLAGS.hidden_size * 2,
                normalizer_fn=tf.contrib.layers.batch_norm)  # (batch_size, context_len, hidden_size * 2)
            coco_layer = CoAttn(self.keep_prob, self.FLAGS.hidden_size, self.FLAGS.hidden_size * 2)
            coco_output = coco_layer.build_graph(coqn_out, self.qn_mask, coco_in, self.context_mask) # coattn_output is shape (batch_size, context_len, hidden_size*2)

        model_reps = tf.concat([bidaf_output, coattn_output, coco_output], 2)  # (batch_size, context_len, hidden_size*6)
        ...
        with vs.variable_scope("EndDist"):
            end_gru_layer = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
            model_reps_end = end_gru_layer.build_graph(model_reps, self.context_mask)
            softmax_layer_end = SimpleSoftmaxLayer()
            self.logits_end, self.probdist_end = softmax_layer_end.build_graph(model_reps_end, self.context_mask)
